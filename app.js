...
import {
  getRowsFromModel, normalizeRow, extractNotesAndCriteria, criteriaCount
} from "./master-data.js";
...
// Faz 8A: Demo ve gerçek veri modu kesin olarak ayrılır.
  ...
  function getOrganizationContextOptions() {
    const map = new Map();
  ...
  function renderRoleSelect() {
    ...
    if (!currentExists && orgOptions[0]) state.orgContext = orgOptions[0].id;
    if (orgSelect) {
  ...
  window.setOrgContext = function(orgId) {
    if (isAuthLocked() && state.role === "auditee") {
      state.orgContext = profileOrg() || state.orgContext;
      renderRoleSelect();
  ...
  function renderSection(sectionId) {
    const map = {
      home: ["Ana Sayfa", "Faz 8A: veri bütünlüğü, güvenlik ve süreç stabilizasyonu."],
  ...
  window.renderMasterLibrary = function() {
  ...
  window.renderMasterRows = function() {
    ...
    document.getElementById("masterStats").innerHTML = `<div class="card" style="margin-bottom:12px"><strong>${area} - ${AREA_LABELS[area]}</strong> <span class="pill blue">${rows.length} PQ</span><span class="pill yellow">${totalCriteria} kriter</span><span class="pill gray">Read-only</span><span class="pill green">${DATA_FILE}</span></div>`;
    document.getElementById("masterRows").innerHTML = rows.map(row => {
  ...
  function renderFormRevisions() {
    const isAdmin = state.role === "admin";
    document.getElementById("formRevisions").innerHTML = `
  ...
  window.createRevisionMeta = async function(area) {
  ...
  function programStatusPill(status) {
  ...
  function auditMethodLabel(value) {
  ...
  function criticalDateForAudit(audit) {
    const candidates = [
  ...
  function getProgramAuditStats(programId) {
    const audits = state.audits.filter(a => a.programId === programId);
    const byStatus = audits.reduce((acc, audit) => { acc[audit.status || "draft"] = (acc[audit.status || "draft"] || 0) + 1; return acc; }, {});
    const forms = new Set();
    audits.forEach(a => (a.selectedForms || []).forEach(f => forms.add(f)));
    return { total: audits.length, byStatus, forms: [...forms] };
  }
  function programSelectOptions(selectedId = "") {
    return state.auditPrograms
      .slice()
      .sort((a,b) => (b.year || 0) - (a.year || 0))
      .map(p => `<option value="${escapeHtml(p.programId)}" ${p.programId === selectedId ? "selected" : ""}>${escapeHtml(p.programId)} - ${escapeHtml(p.name)}</option>`).join("");
  }
  ...
  function programAuditCounts(programId) {
    const audits = state.audits.filter(a => a.programId === programId);
    return {
      total: audits.length,
  ...
  function programCompletionPercent(programId) {
  ...
  window.newAnnualProgram = function() {
    state.editProgramId = null;
    showSection("programCreate");
  };
  window.editAnnualProgram = function(programId) {
  ...
  function renderProgramCreate() {
    ...
    document.getElementById("programCreate").innerHTML = `
  ...
  window.saveAnnualProgram = async function() {
    ...
    await saveRecord(COLLECTIONS.auditPrograms, programId, data);
    state.editProgramId = null;
    await loadData();
    renderAll();
    alert("Yıllık denetim programı kaydedildi.");
    showSection("annualPrograms");
  };
  window.setProgramFilter = function(key, value) {
  ...
  function renderAnnualPrograms() {
    ...
    document.getElementById("annualPrograms").innerHTML = `
  ...
  window.changeProgramStatus = async function(programId, status) {
    ...
    await updateRecord(COLLECTIONS.auditPrograms, programId, { status, [`${status}At`]: new Date().toISOString() });
    await loadData(); renderAll(); showSection("annualPrograms");
  };
  window.archiveProgram = async function(programId) {
  ...
  function renderProgramAddAudit() {
    ...
    document.getElementById("programAddAudit").innerHTML = `
  ...
  window.updateAuditDuePreview = function() {
  ...
  window.checkAllAuditForms = function(checked) { document.querySelectorAll("input[name='auditForms']").forEach(cb => cb.checked = checked); };
  window.newPlannedAudit = function() { state.editAuditId = null; renderProgramAddAudit(); };
  window.editAudit = function(auditId) { state.editAuditId = auditId; showSection("programAddAudit"); };
  window.savePlannedAudit = async function() {
    ...
    await saveRecord(COLLECTIONS.organizations, orgId, { organizationId: orgId, name: orgName, type: "Kuruluş", active: true, createdAt: existing?.createdAt || new Date().toISOString(), updatedAt: new Date().toISOString() });
    const data = {
      ...(existing || {}),
      auditId,
      programId,
    ...
    await saveRecord(COLLECTIONS.audits, auditId, data);
    state.editAuditId = null;
    await loadData();
    renderAll();
    alert("Planlı denetim kaydedildi.");
    showSection("plannedAudits");
  };


  function getAssignment(auditId) {
  ...
  function assignmentCompleteness(auditId) {
  ...
  function assignmentPill(auditId) {
  ...
  function auditFileOptions(selectedId = "") {
    return state.audits
  ...
  function ensureAuditFileSelection() {
  ...
  window.openAuditFile = function(auditId) {
    state.currentAuditFileId = auditId || "";
    if (state.currentAuditFileId) localStorage.setItem("usoap_phase3a_current_audit_file", state.currentAuditFileId);
    showSection("auditFile");
  };

  window.selectAuditFile = function(auditId) {
    state.currentAuditFileId = auditId || "";
    if (state.currentAuditFileId) localStorage.setItem("usoap_phase3a_current_audit_file", state.currentAuditFileId);
    renderAuditFile();
  };
  ...
  function renderAuditFile() {
    ...
    section.innerHTML = `
      ...
      ${canManageAssignments() ? `<div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn success" onclick="saveAuditAssignment('${audit.auditId}')">Heyet ve Dosya Notlarını Kaydet</button><button class="btn" onclick="renderAuditFile()">Değişiklikleri Geri Al</button>${nextActionForAudit(audit) ? `<button class="btn primary" onclick="advanceAudit('${audit.auditId}','${nextActionForAudit(audit).status}')">${escapeHtml(nextActionForAudit(audit).label)}</button>` : ""}</div>` : ""}
    `;
  }

  window.saveAuditAssignment = async function(auditId) {
    const audit = getAudit(auditId);
    ...
    await saveRecord(COLLECTIONS.auditAssignments, assignmentId, data);
    await loadData();
    renderAll();
    state.currentAuditFileId = auditId;
  ...
  function assignmentSummaryHtml(auditId) {
  ...
  function rerenderCurrentAuditList() {
    const cfg = {
  ...
  window.setAuditFilter = function(key, value) { state.auditFilters[key] = value; rerenderCurrentAuditList(); };
  window.resetAuditFilters = function() { state.auditFilters = { search: "", programId: "all", method: "all", type: "all", dateFrom: "", dateTo: "" }; rerenderCurrentAuditList(); };
  ...
  function renderAuditList(sectionId, statuses, title, subtitle) {
    const baseRows = state.audits.filter(a => statuses.includes(a.status));
    const rows = applyAuditFilters(baseRows);
    const dueSoon = rows.filter(a => {
      const d = daysBetween(nextCriticalDate(a));
    ...
    document.getElementById(sectionId).innerHTML = `
      <div class="info-banner"><strong>${escapeHtml(title)}:</strong> ${escapeHtml(subtitle)}</div>
      <div class="summary-strip"><div class="mini-stat"><strong>${baseRows.length}</strong><span>Bu statüde toplam</span></div><div class="mini-stat"><strong>${rows.length}</strong><span>Filtre sonrası</span></div><div class="mini-stat"><strong>${dueSoon}</strong><span>7 gün içinde kritik</span></div><div class="mini-stat"><strong>${overdue}</strong><span>Geciken kritik tarih</span></div></div>
      ${auditFilterToolbar()}
      <div class="list">${rows.map(renderAuditCard).join("") || `<div class="empty">Bu statüde veya filtreye uygun denetim kaydı bulunmuyor.</div>`}</div>`;
  }
  ...
  function nextCriticalDate(audit) {
    const map = {
  ...
  function criticalDateText(audit) {
  ...
  function renderAuditCard(audit) {
    ...
    return `<div class="audit-card">
      ...
      <div><div>${statusPill(audit.status || "draft")} ${audit.riskPriorityLevel ? `<span class="pill ${audit.riskPriorityLevel === "critical" ? "red" : audit.riskPriorityLevel === "high" ? "yellow" : "gray"}">${escapeHtml(audit.riskPriorityLevel)}</span>` : ""}</div><div style="margin-top:6px">${(audit.selectedForms || []).map(a => `<span class="pill gray">${a}</span>`).join("")} ${assignmentPill(audit.auditId)}</div><div class="status-ribbon">${orgResponsePill(audit.auditId)}${preEvalPill(audit.auditId)}${auditResponsePill(audit.auditId)}</div><div class="audit-meta" style="margin-top:6px">Kritik tarih: ${criticalDateText(audit)}</div></div>
      <div class="audit-actions">${isAuditee ? auditeeActions : staffActions}</div>
    </div>`;
  }

  function nextActionForAudit(audit) {
    const map = {
  ...
  window.advanceAudit = async function(auditId, status) {
    const audit = getAudit(auditId);
    if (!audit) return;
    ...
    if (status === 'final_report_preparation') {
      ...
      if (!objection && daysBetween(audit.objectionDueDate) < 0) await createAutoNoObjection(audit);
    ...
    if (status === 'final_report_sent') {
      ...
      const existingReport = getReport(auditId) || {};
      await saveRecord(COLLECTIONS.reports, reportId, {
        ...existingReport,
        reportId,
        reportType: 'final',
        auditId,
        ...
        status: 'sent',
        summaryText: existingReport.summaryText || reportDefaultSummary(audit, auditFindings(auditId)),
        ...
        findingIds: auditFindings(auditId).map(f => f.findingId || f.id),
        sentAt: new Date().toISOString(),
        createdAt: existingReport.createdAt || new Date().toISOString(),
        updatedAt: new Date().toISOString()
      });
      await finalizeReportAndStartCap(audit, new Date().toISOString());
    }
    await loadData();
    renderAll();
    showSection(state.currentSection);
  };
  window.archiveAudit = async function(auditId) {
    if (!confirm("Denetim arşivlensin mi?")) return;
    await updateRecord(COLLECTIONS.audits, auditId, { status: "archived", archivedAt: new Date().toISOString() });
    await loadData(); renderAll(); showSection(state.currentSection);
  };
  window.showAuditDetails = function(auditId) {
    const a = getAudit(auditId);
    if (!a) return;
    const p = getProgram(a.programId);
    showModal(`Denetim Detayı - ${escapeHtml(a.auditId)}`, `<div class="detail-grid"><div class="detail-box"><b>Program</b>${escapeHtml(a.programId || "-")}<br>${escapeHtml(p?.name || "")}</div><div class="detail-box"><b>Kuruluş</b>${escapeHtml(a.organizationName || "-")}<br><small>${escapeHtml(a.organizationId || "-")}</small></div><div class="detail-box"><b>Durum</b>${statusPill(a.status || "planned")}</div><div class="detail-box"><b>Dosya/Hazırlık</b>${assignmentPill(a.auditId)}</div><div class="detail-box"><b>Denetim Şekli/Yöntemi</b>${escapeHtml(AUDIT_TYPE_LABELS[a.auditType] || a.auditType || "-")}<br>${escapeHtml(AUDIT_METHOD_LABELS[a.auditMethod] || a.auditMethod || "-")}</div><div class="detail-box"><b>Tarih/Yer</b>${formatDate(a.plannedStartDate)} - ${formatDate(a.plannedEndDate)}<br>${escapeHtml(a.location || "-")}</div><div class="detail-box"><b>Kontrol Formları</b>${(a.selectedForms || []).map(x => `<span class="pill gray">${x}</span>`).join("") || "-"}</div><div class="detail-box"><b>Kritik Tarihler</b>Kuruluş cevap: ${formatDate(a.auditeeResponseDueDate)}<br>Ön değerlendirme: ${formatDate(a.preEvaluationDueDate)}<br>İtiraz: ${formatDate(a.objectionDueDate)}<br>Nihai rapor: ${formatDate(a.finalReportDueDate)}<br>CAP: ${a.capDueDate ? formatDate(a.capDueDate) : `${a.capDueDays || 45} gün / nihai rapordan sonra`}</div><div class="detail-box"><b>Görev Unvanları</b>${escapeHtml((a.requiredTitles || []).join(", ") || "-")}</div><div class="detail-box"><b>Amaç</b>${escapeHtml(a.objectives || "-")}</div><div class="detail-box"><b>Kapsam</b>${escapeHtml(a.scope || "-")}</div><div class="detail-box"><b>Kriter</b>${escapeHtml(a.criteria || "-")}</div><div class="detail-box"><b>Risk / Öncelik</b>${escapeHtml(a.riskPriorityNote || "-")}</div></div><h3 style="margin-top:16px">Heyet ve Görev Ataması</h3>${assignmentSummaryHtml(a.auditId)}<h3 style="margin-top:16px">Kuruluş Cevabı, Ön Değerlendirme ve Raporlama</h3><div class="detail-grid"><div class="detail-box"><b>Kuruluş Cevabı</b>${orgResponsePill(a.auditId)}</div><div class="detail-box"><b>Ön Değerlendirme</b>${preEvalPill(a.auditId)}</div><div class="detail-box"><b>Denetim Çalışması</b>${auditResponsePill(a.auditId)}</div><div class="detail-box"><b>İtiraz</b>${objectionPill(a.auditId)}</div><div class="detail-box"><b>Nihai Rapor</b>${reportPill(a.auditId)}</div></div><div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn" onclick="closeModal(); openAuditFile('${a.auditId}')">Denetim Dosyası / Heyet</button><button class="btn" onclick="closeModal(); openAuditeeResponse('${a.auditId}')">Kuruluş Cevapları</button><button class="btn" onclick="closeModal(); openPreEvaluation('${a.auditId}')">Ön Değerlendirme</button><button class="btn" onclick="closeModal(); openAuditExecution('${a.auditId}')">Denetim Çalışması</button><button class="btn" onclick="closeModal(); openObjectionProcess('${a.auditId}')">İtiraz Süreci</button><button class="btn" onclick="closeModal(); openFinalReport('${a.auditId}')">Nihai Rapor</button><button class="btn" onclick="closeModal(); editAudit('${a.auditId}')">Plan Bilgilerini Düzenle</button>${nextActionForAudit(a) ? `<button class="btn primary" onclick="closeModal(); advanceAudit('${a.auditId}','${nextActionForAudit(a).status}')">${escapeHtml(nextActionForAudit(a).label)}</button>` : ""}</div>`);
  };
  ...
  function orgResponsePill(auditId) {
    const audit = getAudit(auditId);
  ...
  function responseEligibleAudits() {
  ...
  window.openAuditeeResponse = function(auditId) {
    state.currentAuditeeResponseAuditId = auditId || '';
    if (state.currentAuditeeResponseAuditId) localStorage.setItem('usoap_phase3b_current_response_audit', state.currentAuditeeResponseAuditId);
    showSection('auditeeResponses');
  };

  window.selectAuditeeResponseAudit = function(auditId) {
    state.currentAuditeeResponseAuditId = auditId || '';
    if (state.currentAuditeeResponseAuditId) localStorage.setItem('usoap_phase3b_current_response_audit', state.currentAuditeeResponseAuditId);
  ...
  function renderAuditeeResponses() {
    ...
    section.innerHTML = `
      ...
      <div class="status-ribbon">${duePill}${orgResponsePill(audit.auditId)}${preEvalPill(audit.auditId)}${auditResponsePill(audit.auditId)}</div>
  ...
  window.saveAuditeePreResponse = async function(submit) {
    ...
    if (submit && audit.status === 'waiting_auditee_response') {
      await updateRecord(COLLECTIONS.audits, auditId, { status: 'pre_evaluation', auditeeResponseSubmittedAt: data.submittedAt });
    }
    await loadData();
    renderAll();
    state.currentAuditeeResponseAuditId = auditId;
  ...
  window.openPreEvaluation = function(auditId) {
    state.currentPreEvaluationAuditId = auditId || '';
    if (state.currentPreEvaluationAuditId) localStorage.setItem('usoap_phase3b_current_preeval_audit', state.currentPreEvaluationAuditId);
    showSection('preEvaluation');
  };

  window.selectPreEvaluationAudit = function(auditId) {
    state.currentPreEvaluationAuditId = auditId || '';
    if (state.currentPreEvaluationAuditId) localStorage.setItem('usoap_phase3b_current_preeval_audit', state.currentPreEvaluationAuditId);
    renderPreEvaluation();
  };
  ...
  window.savePreEvaluation = async function(complete) {
    ...
    await saveRecord(COLLECTIONS.auditPreEvaluations, preEvaluationId, data);
    if (complete && ['waiting_auditee_response','pre_evaluation'].includes(audit.status)) {
      await updateRecord(COLLECTIONS.audits, auditId, { status: 'pre_evaluation', preEvaluationCompletedAt: data.completedAt });
    }
    await loadData();
    renderAll();
    state.currentPreEvaluationAuditId = auditId;
  ...
  window.openAuditExecution = function(auditId) {
    state.currentAuditExecutionId = auditId || '';
    if (state.currentAuditExecutionId) localStorage.setItem('usoap_phase4_current_execution_audit', state.currentAuditExecutionId);
    showSection('auditExecution');
  };

  window.selectAuditExecutionAudit = function(auditId) {
    state.currentAuditExecutionId = auditId || '';
    if (state.currentAuditExecutionId) localStorage.setItem('usoap_phase4_current_execution_audit', state.currentAuditExecutionId);
    renderAuditExecution();
  };
  ...
  function criteriaLinesForRow(row) {
  ...
  function criteriaItemsForDisplay(item, row) {
  ...
  function derivePqStatusFromCriteria(criteriaItems, expectedCount) {
  ...
  function auditExecutionCounts(rows, items) {
  ...
  window.toggleExecCard = function(cardId) {
  ...
  window.setAllExecCards = function(open) {
  ...
  window.saveAuditExecution = async function(saveStatus = 'draft') {
    const audit = getAudit(state.currentAuditExecutionId);
    ...
    await saveRecord(COLLECTIONS.auditResponses, auditResponseId, data);
  ...
  async function syncFindingsFromAuditExecution(audit, auditResponse) {
    ...
    for (const row of getAuditRowsForAudit(audit)) {
      ...
      if (item.status === 'NS') {
        ...
        await saveRecord(COLLECTIONS.findings, findingId, finding);
      }
    }
    ...
    for (const f of previous) {
      ...
      if (!activeFindingIds.has(fid)) await updateRecord(COLLECTIONS.findings, fid, { status: 'void', voidedAt: new Date().toISOString(), voidReason: 'PQ artık NS değil veya cevap kaldırıldı.' });
    }
  }
...
${escapeHtml(finding.reviewedEvidence || "-")}</div><div class="grid"><div class="card span-6"><h3>CAP Plan Bilgileri</h3><div class="field"><label>Kök Neden / Değerlendirme</label><textarea id="capRootCause" ${planDisabled}>${escapeHtml(plan.rootCause || "")}</textarea></div><div class="field"><label>Genel Düzeltici Eylem Özeti</label><textarea id="capCorrectionSummary" ${planDisabled}>${escapeHtml(plan.proposedCorrectionSummary || "")}</textarea></div></div><div class="card span-6"><h3>Koordinasyon</h3><div class="field"><label>CAP Koordinatörü / İrtibat</label><input id="capCoordinator" value="${escapeHtml(plan.capCoordinator || "")}" ${planDisabled}></div><div class="field"><label>Plan Notu</label><textarea id="capPlanNote" ${planDisabled}>${escapeHtml(plan.planNote || "")}</textarea></div></div></div><h3 style="margin-top:14px">CAP Adımları</h3>${saveNotice}${stepHtml}${auditeeButtons}${evalPanel}${monitorPanel}</div>`;
  ...
  // Override role selector with auth lock support
  renderRoleSelect = function() {
    ...
    const orgSelect = document.getElementById("orgContextSelect");
    const currentExists = orgOptions.some(o => o.id === state.orgContext);
    if (!currentExists && orgOptions[0] && !(isAuthLocked() && state.role === "auditee")) state.orgContext = orgOptions[0].id;
    if (orgSelect) orgSelect.innerHTML = orgOptions.map(o => `<option value="${escapeHtml(o.id)}" ${o.id === state.orgContext ? "selected" : ""}>${escapeHtml(o.name)} — ${escapeHtml(o.id)}${o.auditCount ? ` (${o.auditCount} denetim)` : ""}</option>`).join("");
    const manual = document.getElementById("orgContextInput");
    if (manual) manual.value = state.orgContext;
    renderAuthState();
  };

  sectionAllowed = function(sectionId) {
    if (sectionId === "notifications") return true;
    if (isAuthLocked() && userIsPending(state.userProfile)) {
  ...
  // Override renderAuditList to apply user-based visibility and clearer deadline warnings.
  renderAuditList = function(sectionId, statuses, title, subtitle) {
    const baseRowsAll = state.audits.filter(a => statuses.includes(a.status));
    const baseRows = visibleAuditsForCurrentUser(baseRowsAll);
    const rows = applyAuditFilters(baseRows);
    ...
    const roleNote = isAuthLocked() ? `<div class="auth-lock-note">Bu liste kullanıcı profilinize göre filtrelenmiştir. Rol: <strong>${escapeHtml(ROLES[state.role] || state.role)}</strong>${state.role === "auditee" ? ` | Kuruluş: <strong>${escapeHtml(state.orgContext)}</strong>` : ""}</div>` : "";
    document.getElementById(sectionId).innerHTML = `${roleNote}<div class="info-banner"><strong>${escapeHtml(title)}:</strong> ${escapeHtml(subtitle)}</div><div class="summary-strip"><div class="mini-stat"><strong>${baseRows.length}</strong><span>Yetkili kayıt</span></div><div class="mini-stat"><strong>${rows.length}</strong><span>Filtre sonrası</span></div><div class="mini-stat"><strong>${dueSoon}</strong><span>7 gün içinde kritik</span></div><div class="mini-stat"><strong>${overdue}</strong><span>Geciken kritik tarih</span></div></div>${auditFilterToolbar()}<div class="list">${rows.map(renderAuditCard).join("") || `<div class="empty">Bu statüde veya filtreye uygun denetim kaydı bulunmuyor.</div>`}</div>`;
  };

  // Override critical date text with stronger visual warnings.
  criticalDateText = function(audit) {
    return deadlineBadge(nextCriticalDate(audit));
  };
...
service cloud.firestore {
  ...
  // Faz 8A: type=module içinde inline HTML olaylarının erişmesi gereken render fonksiyonları.
  window.renderAuditFile = renderAuditFile;
  window.renderAuditeeResponses = renderAuditeeResponses;
